 ...
 * multiple round tournament under the control of a BattleBotsArena object. For
 * instructions
 * on how to create a Bot, see the documentation in
 * the class named <i>Bot</i>. For instructions on how to add Bots to the arena,
 * see the documentation for the <i>fullReset()</i> method in this class.<br>
 * <br>
 *
 ...
 * frames
 * that have been processed. The clock will run faster when the play is sped up
 * to x2,
 * x4, or x8 (do this by mousing over the clock and using the scroll wheel).
 * However, depending
 * on the speed of your computer, you may not get 2x, 4x, or 8x as many frames
 * in that time,
 * so you should test your Bots at regular speed whenever possible. <br>
 * <br>
 *
 * <b>Bot Movement</b> <br>
 * <br>
 ...
 * data structures of the arena, the Bullets, or other Bots. <br>
 * <br>
 *
 * <b>Collisions</b> <br>
 * <br>
 ...
 * point
 * of each Bot is defined as <i>(x+Bot.Radius, y+Bot.RADIUS)</i>, where x and y
 * are
 * the top left corner of a square in which the bot is inscribed. The width
 * and height of each Bot is <i>Bot.RADIUS * 2</i>. Each Bot has a circular
 * collision
 * mask in a radius of Bot.RADIUS from this centre point. Bullets have a single
 * point collision mask (the pixel at the front of the Bullet) and are created
 * one
 * pixel over from the edge of the Bot that fired them, in the middle of the
 * side
 * from which they were fired (i.e. vertical bullets have an x coordinate of
 * <i>x+Bot.Radius</i> and horizontal bullets have a y coordinate of
 * <i>y+Bot.RADIUS</i>).
 * Two bots have collided if the euclidean distance between their centre points
 * is
 * equal to <i>Bot.RADIUS*2</i> or less).<br>
 * <br>
 *
 ...
 * bot's
 * name and team name, message processing, etc.) This is not perfect, but it
 * does
 * give an approximate estimate of how
 * much CPU each Bot is consuming. A small number of points per round are
 * awarded
 * for low CPU usage (see <i>Scoring</i> below). If the cumulative CPU time for
 * any
 ...
 * particular Bot for the remainder of the round.<br>
 * <br>
 *
 * <b>Scoring</b><br>
 * <br>
 ...
 * their bullet
 * hits), and points for each second they stay alive in the round. The round
 * ends
 * after a set time, or after there are 1 or fewer Bots left. If a Bot is left
 * at the
 * end of the round, it is awarded time points based for the entire length the
 * round
 * would have been if it had continued. Point values are increased each each
 * round.<br>
 * <br>
 *
 * In addition to these base points, there are penalties for each exception
 * thrown, and
 * there is a small bonus for low CPU equal to two points minus the number of
 * seconds of
 * CPU time used. This bonus allows ties to be broken (e.g. if two Bots each
 * kill four
 * other Bots and survive to the end of the round, it is usually possible to
 * declare a
 * winner based on CPU time). <br>
 * <br>
 *
 * Five Bots are dropped after each round, until six or fewer Bots are left. So
 * if the
 * game starts with 16 Bots(the default) there will be 3 rounds of play. (16
 * bots, 11 bots,
 * and 6 bots respectively).<br>
 * <br>
 *
 ...
 * be played so that the winning Bot is the one with the highest cumulative
 * score at the
 * end of multiple rounds, or it can be the Bot that wins the final round.<br>
 * <br>
 *
 ...
 * comes up
 * when the Arena is first run is a "Test Mode" screen that allows you to track
 * CPU time
 * and exceptions, view the name and team of each Bot, and check that drawing
 * and
 ...
 * there is a "HumanBot" character that you can control with the keyboard to
 * further
 * test your Bots' performance.<br>
 * <br>
 *
 *
 ...
 *          deep copy
 *          of the BotInfo array for live bots is passed, so that all Bots get
 *          the exact same snapshot of where the Bots are (thanks
 ...
 * @version <br>
 *          2.0 (August 9, 2011) - Converted to an application that can be
 *          JAR'ed -- the mouse wheel was not working well when embedded in a
 *          web page
 * @version <br>
 *          2.1 (November 30, 2011) - Fixed audio bug
 * @author Sam Scott
 *
 */
public class BattleBotArena extends JPanel
	 ...
	 * on one machine
	 * System.currentTimeMillis() shows 10 seconds elapsed while System.nanoTime()
	 * consistently shows
	 * 4.5 seconds elapsed for the same time period. The more reliable millisecond
	 * timing is used
	 * for the game clock, however for timing CPU usage of the Bots, we need a
	 * higher grain than 1ms.
	 * So System.nanoTime() is used, but a correction factor is computed in a thread
	 * spawned at
	 * startup time, and this becomes a multiplier for the number of ns that
	 * System.nanoTime() reports
	 * has elapsed.
	 */
	 ...
	 * arrays - bullets[i]
	 * gives the array of bullets owned by bot[i], and botsInfo[i] gives the public
	 * info
	 * for bot[i].
	 */
	 ...
	 * to
	 * the Bots in getMove(). Done this way so the Bots don't have access to each
	 * others'
	 * internal states. Note that bots, botsInfo, and bullets are related arrays -
	 * bullets[i]
	 * gives the array of bullets owned by bot[i], and botsInfo[i] gives the public
	 * info
	 * for bot[i].
	 ...
	 * System.nanoTime(). See
	 * the documentation of the field "nanoTimeCorrection" for more information. The
	 * thread is
	 * spawned by the init() method, and should be complete and no longer running
	 * after about 20s.
	 */
	public void run() {
		// pause for setup
		...
		if (state == GAME_ON && countDown <= 0 || state == TEST_MODE && ready) {
			...
			if (state != TEST_MODE && (timePlayed >= TIME_LIMIT || botsLeft <= 1)) {
				...
				if (totalOut >= NUM_BOTS - 1) // is this the last round?
				{
					...
					state = WINNER;
				} else if (CUMULATIVE_SCORING) // different message depending on scoring type
							...
							switch (move) {
								...
								case SEND_MESSAGE:
									...
									try {
										...
										if (botsInfo[i].getNumMessages() < TIME_LIMIT / SECS_PER_MSG
												&& state != TEST_MODE)
	...
	private double currentScore(int botNum, boolean gameOver) {
		double score = KILL_SCORE * botsInfo[botNum].getNumKills() * (round + 1.0) / 2
		...
		if (ready) // avoid race condition on startup
		{
			...
			if (state == GAME_PAUSED) {
				...
				g.drawString("- " + df.format(KILL_SCORE * (round + 1.0) / 2) + " points per kill, "
						+ df.format(POINTS_PER_SECOND * (round + 1.0) / 2 * 10) + " points per 10 seconds of survival",
						10, y);
				y += 15;
